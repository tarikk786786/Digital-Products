...
export default function BundleTable() {
  return (
    <section className="py-20 relative z-10 bg-[#0a0a0f]">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/10 bg-white/5">
                  <th className="p-6 text-gray-300 font-medium text-lg w-1/4">Features</th>
                  ...
                  <th className="p-6 text-center w-1/4 bg-gradient-to-b from-[#d4af37]/10 to-transparent border-t-[3px] border-[#d4af37]">
                ...
                {bundleComparison.map((row, idx) => (
                  <tr key={idx} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-6 text-center text-gray-400">
                      {row.ebook === "No" ? <FaTimes className="mx-auto text-red-500/50" /> : 
                       row.ebook.startsWith("Yes") ? <><FaCheck className="mx-auto text-green-500 mb-1" /> {row.ebook !== "Yes" && <span className="text-xs">{row.ebook}</span>}</> : 
                       row.ebook}
                    </td>
                    
                    <td className="p-6 text-center text-gray-400">
                      {row.course === "No" ? <FaTimes className="mx-auto text-red-500/50" /> : 
                       row.course.startsWith("Yes") ? <><FaCheck className="mx-auto text-green-500 mb-1" /> {row.course !== "Yes" && <span className="text-xs">{row.course}</span>}</> : 
                       row.course}
                    </td>
                    
                    <td className="p-6 text-center font-bold text-white bg-[#d4af37]/5">
                      {row.bundle === "No" ? <FaTimes className="mx-auto text-red-500/50" /> : 
                       row.bundle.startsWith("Yes") ? <><FaCheck className="mx-auto text-[#d4af37] mb-1" /> {row.bundle !== "Yes" && <span className="text-xs text-[#d4af37]/80">{row.bundle}</span>}</> : 
